# RACE CONDITIONS — Claude Code Handover Document

_Working title (alternatives: Takeoff, Critical Window). An open-source strategy game about the AI race — teaching the world's hardest coordination problem through play._

> **Purpose of this file:** Complete project handover for Claude Code. Read fully before writing code. This file becomes `docs/HANDOVER.md` in the repo. Companion docs: `ROADMAP.md` (already written) and `SOURCES.md` (bibliography; core set in §11, full 180+ list delivered separately as a deep-research report — paste it in).
...
## 4. The 18 Founding Decisions (condensed; full rationale in chat log → port to `docs/decisions/ADR-001.md`)
...
- Consult the `frontend-design` skill when building UI for distinctive, non-templated visual direction.
...
## 12. Claude Code — First-Run Task List (ordered)

1. **Milestone A — Scaffold (do first):** create repo per §7.2; commit this file to `docs/HANDOVER.md`; copy in `ROADMAP.md`; write README, CONTRIBUTING, GOVERNANCE, CoC, licenses; port §4 to ADR-001; create `data/sources.json` from §11 (verify URLs on fetch; fix or flag broken ones).
...
4. **Milestone D — UI:** Prototype 1 per §6 (text/CSS/emoji aesthetic; consult frontend-design skill); debrief screen; seed sharing; feedback link.
...
_This document + `ROADMAP.md` + the full research bibliography = the complete project base. Build Milestone A, then B. Good luck — make it fun first._
